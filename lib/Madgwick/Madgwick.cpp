...
void MadgwickFilterUpdate(MadgwickData_t* filterData, float w_x, float w_y, float w_z, float a_x, float a_y, float a_z, float m_x, float m_y, float m_z, float deltat)
{
    ...
    J_14or21 = twoSEq_2;    // J_14 negated in matrix multiplication
    J_32 = 2.0f * J_14or21;    // J_32 negated in matrix multiplication
    J_33 = 2.0f * J_11or24;    // J_33 negated in matrix multiplication
    J_41 = twob_zSEq_3;    // J_41 negated in matrix multiplication
    J_42 = twob_zSEq_4;    // J_42 negated in matrix multiplication
    J_43 = 2.0f * twob_xSEq_3 + twob_zSEq_1;    // J_43 negated in matrix multiplication
    J_44 = 2.0f * twob_xSEq_4 - twob_zSEq_2;    // J_44 negated in matrix multiplication
    J_51 = twob_xSEq_4 - twob_zSEq_2;    // J_45 negated in matrix multiplication
    J_52 = twob_xSEq_3 + twob_zSEq_1;    // J_45 negated in matrix multiplication
    J_53 = twob_xSEq_2 + twob_zSEq_4;    // J_53 negated in matrix multiplication
    J_54 = twob_xSEq_1 - twob_zSEq_3;    // J_54 negated in matrix multiplication
    J_61 = twob_xSEq_3;    // J_61 negated in matrix multiplication
    J_62 = twob_xSEq_4 - 2.0f * twob_zSEq_2;    // J_62 negated in matrix multiplication
    J_63 = twob_xSEq_1 - 2.0f * twob_zSEq_3;    // J_63 negated in matrix multiplication
    J_64 = twob_xSEq_2;    // J_64 negated in matrix multiplication
...
void MadgwickGetEuler(MadgwickData_t* filterData) {
    ...
    // Roll (x-axis)
    filterData->roll = atan2f(2.0f * (q0*q1 + q2*q3),
                              1.0f - 2.0f * (q1*q1 + q2*q2)) * 57.2958f;

    // Pitch (y-axis)
    float sinp = 2.0f * (q0*q2 - q3*q1);
    if (fabsf(sinp) >= 1.0f)
        filterData->pitch = copysignf(90.0f, sinp);
    else
        filterData->pitch = asinf(sinp) * 57.2958f;

    // Yaw (z-axis)
    filterData->yaw = atan2f(2.0f * (q0*q3 + q1*q2),
...
float getTrueAz(MadgwickData_t* filterData, float a_z)
{
    ...
    // Gravity component along sensor Z-axis
    float gz = 1.0f - 2.0f * (q1*q1 + q2*q2);

    // True linear acceleration in Z (body Z), gravity removed
    // This is NOT affected by tilting
    return a_z - gz + 1.0f;
}